...
class SticksTest extends AssertionsForJUnit {
	...
	@Test def testCanonicalMove() {
     val r = List(
		...
		val s=new Sticks(r)
        assertEquals(CanonicalMove(1, 0, 1), s.canonicalMove(Move(0,0,1)))
        assertEquals(CanonicalMove(3, 0, 1), s.canonicalMove(Move(1,0,1)))
        assertEquals(CanonicalMove(3, 1, 1), s.canonicalMove(Move(1,1,1)))
        assertEquals(CanonicalMove(3, 0, 2), s.canonicalMove(Move(1,1,2)))
        assertEquals(CanonicalMove(3, 0, 3), s.canonicalMove(Move(1,0,3)))
        assertEquals(CanonicalMove(4, 0, 1), s.canonicalMove(Move(2,1,1)))
        assertEquals(CanonicalMove(4, 1, 1), s.canonicalMove(Move(2,2,1)))
        assertEquals(CanonicalMove(4, 1, 1), s.canonicalMove(Move(2,3,1)))
        assertEquals(CanonicalMove(4, 0, 1), s.canonicalMove(Move(2,4,1)))
        assertEquals(CanonicalMove(4, 0, 2), s.canonicalMove(Move(2,1,2)))
        assertEquals(CanonicalMove(4, 1, 2), s.canonicalMove(Move(2,2,2)))
        assertEquals(CanonicalMove(4, 0, 2), s.canonicalMove(Move(2,3,2)))
        assertEquals(CanonicalMove(4, 0, 3), s.canonicalMove(Move(2,1,3)))
        assertEquals(CanonicalMove(4, 0, 3), s.canonicalMove(Move(2,2,3)))
        assertEquals(CanonicalMove(7, 0, 1), s.canonicalMove(Move(3,0,1)))
        assertEquals(CanonicalMove(7, 0, 3), s.canonicalMove(Move(3,0,3)))
        assertEquals(CanonicalMove(7, 1, 4), s.canonicalMove(Move(3,2,4)))
        assertEquals(CanonicalMove(7, 0, 4), s.canonicalMove(Move(3,3,4)))
        assertEquals(CanonicalMove(7, 0, 3), s.canonicalMove(Move(3,4,3)))
        assertEquals(CanonicalMove(7, 1, 5), s.canonicalMove(Move(3,1,5)))
        assertEquals(CanonicalMove(7, 0, 5), s.canonicalMove(Move(3,2,5)))
